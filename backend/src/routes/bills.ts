...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const populatedBin = await BinRegistration.findById(
        binRegistration._id,
      ).populate("userId");

      // After registration, check if user is linked to at least one bin in this state
      const userBinCount = await BinRegistration.countDocuments({
        userId: req.user!.userId,
        stateCode: state.code,
        isActive: true,
      });
      let registrationMessage = "Bin registered successfully";
      if (userBinCount === 1) {
        registrationMessage +=
          ". You are now linked to a bin in " + state.name + ".";
      } else if (userBinCount > 1) {
        registrationMessage += `. You have ${userBinCount} bins linked in ${state.name}.`;
      } else {
        registrationMessage +=
          ". However, you are not linked to any bin in this state. Please contact support.";
      }
      res.status(201).json({
...
router.get("/bins/search", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (!address && !stateCode) {
      return res
        .status(400)
        .json({ error: "Please provide address or state to search" });
    }
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!binRegistration) {
        console.warn(
          `[BIN LINK] Bin ID not found or inactive: ${binId} (User: ${userId})`,
        );
      ...
      if (binRegistration.userId) {
        if (binRegistration.userId.toString() === userId) {
          console.warn(
            `[BIN LINK] Bin already linked to this user: ${binId} (User: ${userId})`,
          );
          return res
            .status(400)
            .json({ error: "This bin is already linked to your account" });
        } else {
          console.warn(
            `[BIN LINK] Bin already linked to another user: ${binId} (User: ${userId}, LinkedUser: ${binRegistration.userId})`,
          );
          return res
            .status(400)
            .json({ error: "This bin is already linked to another account" });
        }
      }
      ...
      try {
        ...
        console.log(
          "Note: Could not generate initial bill:",
          billError.message,
        );
        // Don't fail the linking if bill generation fails
      }

      const updatedBin = await BinRegistration.findById(
        binRegistration._id,
      ).populate("userId");
      ...
      console.error(
        `[BIN LINK ERROR] User: ${req.user?.userId}, binId: ${req.body?.binId}, error:`,
        error,
      );
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!binRegistration) {
        return res
          .status(404)
          .json({ error: "Bin not found or not authorized" });
      }
...
router.get("/:billId/download", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=bill-${bill.billNumber}.html`,
    );
    res.send(html);
  } catch (error) {
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        ...
        req.user!.stateCode !== state.code
      ) {
        return res
          .status(403)
          .json({ error: "You can only create bills for your state" });
      }
      ...
      res
        .status(201)
        .json({ message: "Bill created successfully", bill: populatedBill });
    } catch (error) {
...
router.patch(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        req.user!.role === "STATE_ADMIN" &&
        req.user!.stateCode !== bill.stateCode
      ) {
        return res
          .status(403)
          .json({ error: "You can only update bills for your state" });
      }
