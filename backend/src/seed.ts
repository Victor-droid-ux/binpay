...
async function seed() {
  try {
    ...
    for (const stateData of statesData) {
      ...
      console.log(
        `  📍 Created ${state.lgas.length} LGAs and ${state.zones.length} zones`,
      );

      // Create state admin (use same password as super admin for consistency)
      const stateAdminPassword =
        process.env.SUPER_ADMIN_PASSWORD || "Admin123!@#";
      const hashedStateAdminPassword = await bcrypt.hash(
        stateAdminPassword,
        10,
      );
      ...
      // Seed sample bin registrations for every state
      const sampleLGA =
